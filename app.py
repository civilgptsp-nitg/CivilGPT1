...
import time # Added for time.sleep in a non-rerun scenario
if needed
 
#
==============================================================================
...
 
class CONSTANTS:
    # EXTENDED: Added M60-M100 grades for HPC mode
    GRADE_STRENGTH = {
        "M10": 10, "M15": 15, "M20": 20, "M25": 25, "M30": 30, "M35": 35, "M40": 40, "M45": 45, "M50": 50,
        "M60": 60, "M70": 70, "M80": 80, "M90": 90, "M100": 100
    }
...
    # EXTENDED: Added binder ranges for HPC grades M60-M100
    BINDER_RANGES = {
...
            # FIX: Use .get() for robustness to handle new HPC grades without breaking
            fck = CONSTANTS.GRADE_STRENGTH.get(grade)
            if fck is None: continue
...
    result = {}
    
    # MODIFIED: Include all HPC grades (M10-M100)
    all_grades = "|".join([g[1:] for g in CONSTANTS.GRADE_STRENGTH.keys()])
    grade_match = re.search(r"\bM\s*(" + all_grades + r")\b", text, re.IGNORECASE)
    if grade_match:
        result["grade"] = "M" + grade_match.group(1).upper()
    
...
 
    # MODIFIED: Include all grades M10-M100 in the system prompt
    all_grades = list(CONSTANTS.GRADE_STRENGTH.keys())
    
    system_prompt = f"""
...
    - "grade": (String) Must be one of {all_grades}
...
        
        cleaned_data = {}
        # MODIFIED: Check against the full GRADE_STRENGTH keys
...
    if cement > 500 and meta.get("grade") not in ["M60", "M70", "M80", "M90", "M100"]: warnings.append(f"High cement content ({cement:.1f} kg/m³). Increases cost, shrinkage, and CO₂.")
    if not 140 <= water <= 220 and meta.get("grade") not in ["M60", "M70", "M80", "M90", "M100"]: warnings.append(f"Water content ({water:.1f} kg/m³) is outside the typical range of 140-220 kg/m³ for standard grades.")
...
    if sp > 20 and meta.get("grade") not in ["M60", "M70", "M80", "M90", "M100"]: warnings.append(f"Superplasticizer dosage ({sp:.1f} kg/m³) is unusually high for standard grades.")
    
    # ADDED: HPC specific check/note for very high binder content which is necessary
    if cement + meta.get("flyash", 0) + meta.get("ggbs", 0) > 550 and meta.get("grade") in ["M60", "M70", "M80", "M90", "M100"]: 
        warnings.append(f"High total binder content ({cement + meta.get('flyash', 0) + meta.get('ggbs', 0):.1f} kg/m³) is typical and required for HPC grades.")
    
    return warnings
 
...
    questions = {
       "grade": f"What concrete grade do you need (e.g., M30, M50, M80)? Current available grades: {', '.join(CONSTANTS.GRADE_STRENGTH.keys())}",
...
        
        # Safely find indices for comparison, defaulting to -1 if grade is missing
        current_grade_idx = grade_order.index(inputs["grade"]) if inputs["grade"] in grade_order else -1
        min_grade_req_idx = grade_order.index(min_grade_req) if min_grade_req in grade_order else -1
        
        if current_grade_idx != -1 and min_grade_req_idx != -1 and current_grade_idx < min_grade_req_idx:
...
        c1, c2, c3, c4 = st.columns(4)
        
        # ADDED: Get the list of grades based on HPC toggle state
        standard_grades = list(CONSTANTS.GRADE_STRENGTH.keys())[:9] # M10-M50
        hpc_grades = list(CONSTANTS.GRADE_STRENGTH.keys())[9:] # M60-M100
        all_grades = standard_grades + hpc_grades
        
        # ADDED: HPC Toggle
        hpc_enabled = st.checkbox("🧪 Enable HPC (M60–M100)", False, key="hpc_toggle", help="Enable High-Performance Concrete grades from M60 up to M100.")
 
        if hpc_enabled:
            grade_options = all_grades
            default_grade_idx = all_grades.index("M60") # Default to M60 if HPC is on
        else:
            grade_options = standard_grades
            default_grade_idx = standard_grades.index("M30") # Default to M30 if HPC is off
        
        # FIX: Ensure grade default index is safe if session state holds a value outside current options
        current_grade_value = st.session_state.get("grade", "M30" if not hpc_enabled else "M60")
        try:
            safe_index = grade_options.index(current_grade_value)
        except ValueError:
            safe_index = default_grade_idx
        
        with c1:
            grade = st.selectbox("Concrete Grade", grade_options, index=safe_index, help="Target characteristic compressive strength at 28 days.", key="grade")
        with c2:
...
 
    # MODIFIED: Update CLARIFICATION_WIDGETS to include all grades
    all_grades = list(CONSTANTS.GRADE_STRENGTH.keys())
    CLARIFICATION_WIDGETS = {
        "grade": lambda v: st.selectbox("Concrete Grade", all_grades,
            index=all_grades.index(v) if v in all_grades else all_grades.index("M30")),
        "exposure": lambda v: st.selectbox("Exposure Condition", list(CONSTANTS.EXPOSURE_WB_LIMITS.keys()),
            index=list(CONSTANTS.EXPOSURE_WB_LIMITS.keys()).index(v) if v in CONSTANTS.EXPOSURE_WB_LIMITS else 2),
