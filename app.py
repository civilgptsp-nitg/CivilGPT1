...
class CONSTANTS:
    ...
    PURPOSE_PROFILES = {
        "General": {
            ...
            "wb_limit": 1.0, "scm_limit": 0.5, "min_binder": 0.0, "max_binder": 600.0,
            "target_slump_range": (50, 150), "min_fines_content": 300, "max_sp_frac": 0.03,
            "preferred_scm_types": ["flyash", "ggbs"], "strength_priority": "medium",
            "durability_priority": "medium", "workability_priority": "medium", "cost_priority": "medium",
            "sustainability_priority": "medium", "weights": {"co2": 0.4, "cost": 0.4, "purpose": 0.2}
        },
        "Slab": {
            "description": "Prioritizes workability (slump) and cost-effectiveness. Strength is often not the primary driver.",
            "wb_limit": 0.55, "scm_limit": 0.5, "min_binder": 300, "max_binder": 450,
            "target_slump_range": (75, 150), "min_fines_content": 350, "max_sp_frac": 0.025,
            "preferred_scm_types": ["flyash"], "strength_priority": "low", "durability_priority": "medium",
            "workability_priority": "high", "cost_priority": "high", "sustainability_priority": "medium",
            "weights": {"co2": 0.3, "cost": 0.5, "purpose": 0.2}
        },
        "Beam": {
            "description": "Prioritizes strength (modulus) and durability. Often heavily reinforced.",
            "wb_limit": 0.50, "scm_limit": 0.4, "min_binder": 320, "max_binder": 500,
            "target_slump_range": (75, 125), "min_fines_content": 380, "max_sp_frac": 0.03,
            "preferred_scm_types": ["ggbs", "flyash"], "strength_priority": "high", "durability_priority": "high",
            "workability_priority": "medium", "cost_priority": "low", "sustainability_priority": "medium",
        ...
        "Column": {
            ...
            "wb_limit": 0.45, "scm_limit": 0.35, "min_binder": 340, "max_binder": 550,
            "target_slump_range": (100, 175), "min_fines_content": 400, "max_sp_frac": 0.035,
            "preferred_scm_types": ["ggbs"], "strength_priority": "very_high", "durability_priority": "very_high",
            "workability_priority": "high", "cost_priority": "low", "sustainability_priority": "medium",
        ...
        "Pavement": {
            ...
            "wb_limit": 0.45, "scm_limit": 0.4, "min_binder": 340, "max_binder": 480,
            "target_slump_range": (25, 75), "min_fines_content": 370, "max_sp_frac": 0.02,
            "preferred_scm_types": ["flyash"], "strength_priority": "high", "durability_priority": "very_high",
            "workability_priority": "low", "cost_priority": "high", "sustainability_priority": "medium",
        ...
        "Precast": {
            ...
            "wb_limit": 0.45, "scm_limit": 0.3, "min_binder": 360, "max_binder": 520,
            "target_slump_range": (50, 100), "min_fines_content": 390, "max_sp_frac": 0.03,
            "preferred_scm_types": [], "strength_priority": "high", "durability_priority": "medium",
            "workability_priority": "medium", "cost_priority": "high", "sustainability_priority": "low",
        ...
        "RPC/HPC": {
            ...
            "wb_limit": 0.35, "scm_limit": 0.25, "min_binder": 450, "max_binder": 800,
            "target_slump_range": (200, 250), "min_fines_content": 450, "max_sp_frac": 0.05,
            "preferred_scm_types": ["silica_fume", "ggbs"], "strength_priority": "very_high",
            "durability_priority": "very_high", "workability_priority": "high", "cost_priority": "very_low",
            "sustainability_priority": "low", "weights": {"co2": 0.4, "cost": 0.1, "purpose": 0.5}
        }
    }
    PRIORITY_WEIGHTS = {"very_low": 0.2, "low": 0.4, "medium": 0.6, "high": 0.8, "very_high": 1.0}
    HPC_OPTIONS = {
        "silica_fume": {"max_frac": 0.10, "water_demand_multiplier": 1.05, "sp_effectiveness_boost": 1.2,
                       "co2_factor": 0.1, "cost_factor": 15.0, "density": 2200.0}
    }
...
def _normalize_columns(df, column_map):
    ...
    for col in df.columns:
        ...
        if norm_col not in norm_cols: norm_cols[norm_col] = col
    rename_dict = {}
...
@st.cache_data
def load_purpose_profiles(filepath=None): return CONSTANTS.PURPOSE_PROFILES

def adjust_water_for_HPC(base_water: float, sf_frac: float, hpc_options: dict) -> float:
    if sf_frac > 0: return base_water * (1 + (hpc_options["silica_fume"]["water_demand_multiplier"] - 1) * sf_frac)
    return base_water

def adjust_sp_for_HPC(base_sp: float, sf_frac: float, hpc_options: dict) -> float:
    if sf_frac > 0: return base_sp * hpc_options["silica_fume"]["sp_effectiveness_boost"]
    return base_sp
...
def evaluate_purpose_specific_metrics(candidate_meta: dict, purpose: str) -> dict:
    try:
        ...
        if sf_frac > 0.05: modulus_proxy = 5700 * np.sqrt(fck_target)
        else: modulus_proxy = 5000 * np.sqrt(fck_target)
        shrinkage_risk_index = (binder * water) / 10000.0
        fatigue_proxy = (1.0 - wb) * (binder / 1000.0)
        if sf_frac > 0.02: fatigue_proxy *= 1.2
        ...
        if wb > purpose_profile.get('wb_limit', 1.0): compliance_score -= (wb - purpose_profile['wb_limit']) * 100
        scm_total = candidate_meta.get('scm_total_frac', 0.0)
        if scm_total > purpose_profile.get('scm_limit', 0.5): compliance_score -= (scm_total - purpose_profile['scm_limit']) * 50
        if binder < purpose_profile.get('min_binder', 0.0): compliance_score -= (purpose_profile['min_binder'] - binder) * 0.5
        if binder > purpose_profile.get('max_binder', 600.0): compliance_score -= (binder - purpose_profile['max_binder']) * 0.2
        if fines_content < purpose_profile.get('min_fines_content', 300): compliance_score -= (purpose_profile['min_fines_content'] - fines_content) * 0.1
        if sp_frac > purpose_profile.get('max_sp_frac', 0.03): compliance_score -= (sp_frac - purpose_profile['max_sp_frac']) * 500
        compliance_score = max(0, compliance_score)
        return {
            "estimated_modulus_proxy (MPa)": round(modulus_proxy, 0), "shrinkage_risk_index": round(shrinkage_risk_index, 2),
            "pavement_fatigue_proxy": round(fatigue_proxy, 2), "hpc_strength_efficiency": round(hpc_strength_index, 2) if sf_frac > 0 else None,
            "pumpability_assessment": "Good" if pumpable else "Marginal", "fines_content_ok": fines_ok, "sp_dosage_ok": sp_ok,
            "workability_score": round(workability_score, 1), "strength_efficiency": round(strength_efficiency, 2),
            "durability_index": round(durability_index, 1), "cost_efficiency": round(cost_per_mpa, 2),
            "sustainability_score": round(sustainability_score, 1), "purpose_compliance_score": round(compliance_score, 1)
        }
    except Exception as e: return {"estimated_modulus_proxy (MPa)": None, "shrinkage_risk_index": None, "pavement_fatigue_proxy": None, "error": str(e)}}
...
def compute_purpose_penalty(candidate_meta: dict, purpose_profile: dict) -> float:
    ...
    try:
        ...
        if current_wb > wb_limit: penalty += (current_wb - wb_limit) * 2000
        if current_scm > scm_limit: penalty += (current_scm - scm_limit) * 500
        if current_binder < min_binder: penalty += (min_binder - current_binder) * 2
        elif current_binder > max_binder: penalty += (current_binder - max_binder) * 0.5
        if current_slump < target_slump_min: penalty += (target_slump_min - current_slump) * 1.5
        elif current_slump > target_slump_max: penalty += (current_slump - target_slump_max) * 1.0
        if current_fines < min_fines_content: penalty += (min_fines_content - current_fines) * 0.3
        if current_sp_frac > max_sp_frac: penalty += (current_sp_frac - max_sp_frac) * 1000
        sf_frac = float(candidate_meta.get('sf_frac', 0.0))
        if sf_frac > 0:
            if current_sp_frac < 0.015: penalty += (0.015 - current_sp_frac) * 2000
            if current_wb > 0.40: penalty += (current_wb - 0.40) * 1000
            if current_fines < 400: penalty += (400 - current_fines) * 0.5
        ...
        if strength_efficiency < 0.3: penalty += (0.3 - strength_efficiency) * 500 * strength_weight
        if current_wb > 0.5: penalty += (current_wb - 0.5) * 300 * durability_weight
        ...
        if current_cost > 5000: penalty += (current_cost - 5000) * 0.01 * cost_weight
        current_co2 = float(candidate_meta.get('co2_total', 0))
        if current_co2 > 400: penalty += (current_co2 - 400) * 0.1 * sustainability_weight
        return float(max(0.0, penalty))
    except Exception as e: return 1000.0
...
def compute_purpose_penalty_vectorized(df: pd.DataFrame, purpose_profile: dict) -> pd.Series:
    ...
    if 'target_slump' in df.columns: target_slump_series = df['target_slump']
    else: target_slump_series = pd.Series((target_slump_min + target_slump_max) / 2, index=df.index)
    penalty += ((target_slump_min - target_slump_series).clip(lower=0) * 1.5 + (target_slump_series - target_slump_max).clip(lower=0) * 1.0)
    ...
    if 'fck_target' not in df.columns and 'grade' in df.columns:
        ...
        except: strength_efficiency_series = pd.Series(0.3, index=df.index)
    elif 'fck_target' in df.columns: strength_efficiency_series = df['fck_target'] / (df['binder'] / 100).replace(0, 1)
    else: strength_efficiency_series = pd.Series(0.3, index=df.index)
    ...
    if 'cost_total' in df.columns: penalty += ((df['cost_total'] - 5000).clip(lower=0) * 0.01 * cost_weight)
    if 'co2_total' in df.columns: penalty += ((df['co2_total'] - 400).clip(lower=0) * 0.1 * sustainability_weight)
...
def load_data(materials_file=None, emissions_file=None, cost_file=None):
    ...
    if "Material" in materials.columns: materials["Material"] = materials["Material"].astype(str).str.strip()
    ...
    if "Material" in emissions.columns: emissions["Material"] = emissions["Material"].astype(str).str.strip()
    ...
    if "Material" in costs.columns: costs["Material"] = costs["Material"].astype(str).str.strip()
...
def pareto_front(df, x_col="cost", y_col="co2", z_col="purpose_penalty"):
    ...
    for _, row in sorted_df.iterrows():
        ...
        for pareto_point in pareto_points:
            if (pareto_point[x_col] <= row[x_col] and pareto_point[y_col] <= row[y_col] and pareto_point[z_col] <= row[z_col] and
                (pareto_point[x_col] < row[x_col] or pareto_point[y_col] < row[y_col] or pareto_point[z_col] < row[z_col])):
                dominated = True
                break
        if not dominated:
            pareto_points = [p for p in pareto_points if not (
                row[x_col] <= p[x_col] and row[y_col] <= p[y_col] and row[z_col] <= p[z_col] and
                (row[x_col] < p[x_col] or row[y_col] < p[y_col] or row[z_col] < p[z_col])
            )]
...
def run_lab_calibration(lab_df):
    ...
    for _, row in lab_df.iterrows():
        try:
            ...
            results.append({
                "Grade": grade, "Exposure": row.get('exposure', 'N/A'), "Slump (mm)": row.get('slump', 'N/A'),
                "Lab Strength (MPa)": actual_strength, "Predicted Target Strength (MPa)": predicted_strength,
...
{{"grade": "M30", "exposure": "Marine", "nom_max": 20, "target_slump": 100, "cement_type": "PPC", "purpose": "Column"}}
    ...
    try:
        ...
        if isinstance(parsed_json.get("enable_hpc"), bool): cleaned_data["enable_hpc"] = parsed_json["enable_hpc"]
        return cleaned_data
    except Exception as e:
...
def compliance_checks(mix_df, meta, exposure):
    ...
    except: checks["HPC specific checks"] = False
    derived = {
        "w/b used": round(float(meta.get("w_b", 0.0)), 3), "cementitious (kg/m³)": round(float(meta.get("cementitious", 0.0)), 1),
        ...
        "water target (kg/m³)": round(float(meta.get("water_target", 0.0)), 1), "cement (kg/m³)": round(float(meta.get("cement", 0.0)), 1),
        "fly ash (kg/m³)": round(float(meta.get("flyash", 0.0)), 1), "GGBS (kg/m³)": round(float(meta.get("ggbs", 0.0)), 1),
        "silica fume (kg/m³)": round(float(meta.get("silica_fume", 0.0)), 1), "fine agg (kg/m³)": round(float(meta.get("fine", 0.0)), 1),
        "coarse agg (kg/m³)": round(float(meta.get("coarse", 0.0)), 1), "SP (kg/m³)": round(float(meta.get("sp", 0.0)), 2),
    ...
    if "purpose" in meta and meta["purpose"] != "General":
        derived.update({"purpose": meta["purpose"], "purpose_penalty": meta.get("purpose_penalty"),
...
def get_compliance_reasons(mix_df, meta, exposure):
    ...
    try:
        ...
        total_mass = float(mix_df["Quantity (kg/m3)"].sum())
        if not (min_limit <= total_mass <= max_limit): reasons.append(f"Unit weight outside range ({total_mass:.1f} kg/m³ not in {min_limit:.0f}-{max_limit:.0f} kg/m³)")
    except: reasons.append("Failed unit weight check (parsing error)")
    try:
        sf_frac = float(meta.get("sf_frac", 0.0))
        if sf_frac > 0:
            sp_frac = float(meta.get("sp", 0)) / float(meta["cementitious"]) if float(meta["cementitious"]) > 0 else 0
            if sp_frac < 0.015: reasons.append(f"Insufficient SP for silica fume ({sp_frac:.1%} < 1.5%)")
            fines_content = float(meta.get("fine", 0)) + float(meta["cementitious"]) * sf_frac
            if fines_content < CONSTANTS.HPC_MIN_FINES_CONTENT: reasons.append(f"Insufficient fines for HPC pumpability ({fines_content:.0f} kg/m³ < {CONSTANTS.HPC_MIN_FINES_CONTENT} kg/m³)")
    except: pass
...
def get_compliance_reasons_vectorized(df: pd.DataFrame, exposure: str) -> pd.Series:
    ...
    reasons += np.where(df['scm_total_frac'] > 0.50, "SCM fraction exceeds limit (" + (df['scm_total_frac'] * 100).round(0).astize(str) + "% > 50%); ", "")
    reasons += np.where(~((df['total_mass'] >= 2200) & (df['total_mass'] <= 2600)), "Unit weight outside range (" + df['total_mass'].round(1).astize(str) + " not in 2200-2600); ", "")
    sf_frac_series = df.get('sf_frac', pd.Series(0.0, index=df.index))
    sp_frac_series = df['sp'] / df['binder'].replace(0, 1)
    fines_content_series = df['fine_wet'] + df['binder'] * sf_frac_series
    reasons += np.where((sf_frac_series > 0) & (sp_frac_series < 0.015), "Insufficient SP for silica fume (" + (sp_frac_series * 100).round(1).astize(str) + "% < 1.5%); ", "")
    reasons += np.where((sf_frac_series > 0) & (fines_content_series < CONSTANTS.HPC_MIN_FINES_CONTENT), "Insufficient fines for HPC pumpability (" + fines_content_series.round(0).astize(str) + " kg/m³ < " + str(CONSTANTS.HPC_MIN_FINES_CONTENT) + " kg/m³); ", "")
...
def generate_mix(grade, exposure, nom_max, target_slump, agg_shape, fine_zone, emissions, costs, cement_choice, material_props, use_sp=True, sp_reduction=0.18, optimize_cost=False, wb_min=0.35, wb_steps=6, max_flyash_frac=0.3, max_ggbs_frac=0.5, scm_step=0.1, fine_fraction_override=None, purpose='General', purpose_profile=None, purpose_weights=None, enable_purpose_optimization=False, enable_hpc=False, hpc_options=None, st_progress=None):
    ...
    else: w_b_limit = float(CONSTANTS.EXPOSURE_WB_LIMITS[exposure])
    ...
    if hpc_options is None: hpc_options = CONSTANTS.HPC_OPTIONS
    ...
    materials_to_calc = [norm_cement_choice, CONSTANTS.NORM_FLYASH, CONSTANTS.NORM_GGBS, CONSTANTS.NORM_SILICA_FUME, CONSTANTS.NORM_WATER, CONSTANTS.NORM_SP, CONSTANTS.NORM_FINE_AGG, CONSTANTS.NORM_COARSE_AGG]
    ...
    if enable_hpc:
        grid_df['water_adjusted'] = grid_df.apply(lambda row: adjust_water_for_HPC(target_water, row['sf_frac'], hpc_options), axis=1)
        grid_df['binder_for_strength'] = grid_df['water_adjusted'] / grid_df['wb_input']
    else: grid_df['water_adjusted'] = target_water
    ...
    if enable_hpc:
        sp_multiplier = np.where(grid_df['sf_frac'] > 0, hpc_options["silica_fume"]["sp_effectiveness_boost"], 1.0)
        grid_df['sp'] = base_sp_series * sp_multiplier
    else: grid_df['sp'] = base_sp_series
    if st_progress: st_progress.progress(0.3, text="Calculating aggregate proportions...")
    if fine_fraction_override is not None and fine_fraction_override > 0.3: grid_df['coarse_agg_fraction'] = 1.0 - fine_fraction_override
    else: grid_df['coarse_agg_fraction'] = get_coarse_agg_fraction_vectorized(nom_max, fine_zone, grid_df['w_b'])
    grid_df['fine_ssd'], grid_df['coarse_ssd'] = compute_aggregates_vectorized(grid_df['binder'], grid_df['water_adjusted'], grid_df['sp'], grid_df['coarse_agg_fraction'], nom_max, density_fa, density_ca)
    ...
    grid_df['co2_total'] = (grid_df['cement'] * co2_factors.get(norm_cement_choice, 0.0) + grid_df['flyash'] * co2_factors.get(CONSTANTS.NORM_FLYASH, 0.0) + grid_df['ggbs'] * co2_factors.get(CONSTANTS.NORM_GGBS, 0.0) + grid_df['silica_fume'] * co2_factors.get(CONSTANTS.NORM_SILICA_FUME, 0.0) + grid_df['water_final'] * co2_factors.get(CONSTANTS.NORM_WATER, 0.0) + grid_df['sp'] * co2_factors.get(CONSTANTS.NORM_SP, 0.0) + grid_df['fine_wet'] * co2_factors.get(CONSTANTS.NORM_FINE_AGG, 0.0) + grid_df['coarse_wet'] * co2_factors.get(CONSTANTS.NORM_COARSE_AGG, 0.0))
    grid_df['cost_total'] = (grid_df['cement'] * cost_factors.get(norm_cement_choice, 0.0) + grid_df['flyash'] * cost_factors.get(CONSTANTS.NORM_FLYASH, 0.0) + grid_df['ggbs'] * cost_factors.get(CONSTANTS.NORM_GGBS, 0.0) + grid_df['silica_fume'] * cost_factors.get(CONSTANTS.NORM_SILICA_FUME, 0.0) + grid_df['water_final'] * cost_factors.get(CONSTANTS.NORM_WATER, 0.0) + grid_df['sp'] * cost_factors.get(CONSTANTS.NORM_SP, 0.0) + grid_df['fine_wet'] * cost_factors.get(CONSTANTS.NORM_FINE_AGG, 0.0) + grid_df['coarse_wet'] * cost_factors.get(CONSTANTS.NORM_COARSE_AGG, 0.0))
    if st_progress: st_progress.progress(0.7, text="Checking compliance and purpose-fit...")
    grid_df['total_mass'] = (grid_df['cement'] + grid_df['flyash'] + grid_df['ggbs'] + grid_df['silica_fume'] + grid_df['water_final'] + grid_df['sp'] + grid_df['fine_wet'] + grid_df['coarse_wet'])
    ...
    if enable_hpc:
        ...
        grid_df['feasible'] = (grid_df['check_wb'] & grid_df['check_min_cem'] & grid_df['check_scm'] & grid_df['check_unit_wt'] & grid_df['check_hpc_fines'] & grid_df['check_hpc_sp'])
    else: grid_df['feasible'] = (grid_df['check_wb'] & grid_df['check_min_cem'] & grid_df['check_scm'] & grid_df['check_unit_wt'])
    ...
    if enable_hpc and best_meta_series['sf_frac'] > 0: best_mix_dict["Silica Fume"] = best_meta_series['silica_fume']
    best_df = evaluate_mix(best_mix_dict, emissions, costs)
    best_meta = best_meta_series.to_dict()
    best_meta.update({
        "cementitious": best_meta_series['binder'], "water_target": target_water, "water_adjusted": best_meta_series.get('water_adjusted', target_water),
        "fine": best_meta_series['fine_wet'], "coarse": best_meta_series['coarse_wet'], "silica_fume": best_meta_series.get('silica_fume', 0.0),
        "sf_frac": best_meta_series.get('sf_frac', 0.0), "grade": grade, "exposure": exposure, "nom_max": nom_max,
        "slump": target_slump, "binder_range": (min_b_grade, max_b_grade), "material_props": material_props,
        "enable_hpc": enable_hpc, "purpose_metrics": evaluate_purpose_specific_metrics(best_meta, purpose)
    })
    trace_df = grid_df.rename(columns={"w_b": "wb", "cost_total": "cost", "co2_total": "co2"})
...
def generate_baseline(grade, exposure, nom_max, target_slump, agg_shape, fine_zone, emissions, costs, cement_choice, material_props, use_sp=True, sp_reduction=0.18, purpose='General', purpose_profile=None, enable_hpc=False):
    ...
    meta = {
        ...
        "coarse_agg_fraction": coarse_agg_frac, "material_props": material_props, "binder_range": (min_b_grade, max_b_grade), "enable_hpc": enable_hpc
    }
    if purpose_profile is None: purpose_profile = CONSTANTS.PURPOSE_PROFILES.get(purpose, CONSTANTS.PURPOSE_PROFILES['General'])
    meta.update({
        "purpose": purpose, "purpose_metrics": evaluate_purpose_specific_metrics(meta, purpose),
        "purpose_penalty": compute_purpose_penalty(meta, purpose_profile), "composite_score": np.nan
    })
    return df, meta
...
def apply_parser(user_text, current_inputs, use_llm_parser=False):
    ...
    if "exposure" in parsed and parsed["exposure"] in CONSTANTS.EXPOSURE_WB_LIMITS: updated["exposure"] = parsed["exposure"]; messages.append(f"✅ Parser set Exposure to **{parsed['exposure']}**")
    ...
    if "enable_hpc" in parsed: updated["enable_hpc"] = parsed["enable_hpc"]; messages.append(f"✅ Parser set HPC to **{parsed['enable_hpc']}**")
...
def display_mix_details(title, df, meta, exposure):
    ...
    if purpose != "General" or enable_hpc:
        ...
        if enable_hpc:
            ...
            if "purpose_metrics" in meta and meta["purpose_metrics"].get("pumpability_assessment"): c7.metric("📊 Pumpability", meta["purpose_metrics"]["pumpability_assessment"])
    ...
    if (purpose != "General" or enable_hpc) and "purpose_metrics" in meta:
        with st.expander(f"Show Enhanced Purpose-Specific Metrics ({purpose})"): st.json(meta["purpose_metrics"])
...
def run_generation_logic(inputs: dict, emissions_df: pd.DataFrame, costs_df: pd.DataFrame, purpose_profiles_data: dict, st_progress=None):
    try:
        ...
        if 'max_sf_frac' in calibration_kwargs: del calibration_kwargs['max_sf_frac']
        if 'use_hpc_presets' in calibration_kwargs: del calibration_kwargs['use_hpc_presets']
        ...
        if st_progress:
            if enable_hpc: st.info(f"🧪 Running High Performance Concrete optimization for **{purpose}**.", icon="🔬")
            elif enable_purpose_opt: st.info(f"🚀 Running composite optimization for **{purpose}**.", icon="🛠️")
            else: st.info(f"Running single-objective optimization for **{inputs.get('optimize_for', 'CO₂ Emissions')}**.", icon="⚙️")
        ...
        opt_df, opt_meta, trace = generate_mix(
            ...
            emissions_df, costs_df, inputs["cement_choice"], material_props=inputs["material_props"],
            use_sp=inputs["use_sp"], optimize_cost=inputs["optimize_cost"], purpose=purpose, purpose_profile=purpose_profile,
            purpose_weights=purpose_weights, enable_purpose_optimization=enable_purpose_opt, enable_hpc=enable_hpc,
        ...
        base_df, base_meta = generate_baseline(
            inputs["grade"], inputs["exposure"], inputs["nom_max"], inputs["target_slump"], inputs["agg_shape"], inputs["fine_zone"],
            emissions_df, costs_df, inputs["cement_choice"], material_props=inputs["material_props"],
            use_sp=inputs.get("use_sp", True), purpose=purpose, purpose_profile=purpose_profile, enable_hpc=enable_hpc
        )
        ...
        if opt_df is None or base_df is None:
            ...
            for m in (opt_meta, base_meta):
                m.update({"fck": fck, "fck_target": round(fck_target, 1), "stddev_S": S, "qc_level": inputs.get("qc_level", "Good"),
...
def run_manual_interface(purpose_profiles_data: dict, materials_df: pd.DataFrame, emissions_df: pd.DataFrame, costs_df: pd.DataFrame):
    ...
    with st.expander("⚙️ Advanced Manual Input: Detailed Parameters and Libraries", expanded=False):
        ...
        enable_hpc = st.toggle("Enable HPC (M60–M100)", value=False, key="enable_hpc_toggle", help="When enabled, adds high-performance concrete grades (M60–M100) to the grade options and enables silica fume optimization.")
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            grade_list = list(CONSTANTS.GRADE_STRENGTH.keys())
            if not st.session_state.get("enable_hpc_toggle", False): grade_list = [g for g in grade_list if int(g[1:]) <= 50]
            grade = st.selectbox("Concrete Grade", grade_list, index=grade_list.index("M30"), key="grade")
        with c2: exposure = st.selectbox("Exposure Condition", list(CONSTANTS.EXPOSURE_WB_LIMITS.keys()), index=2, help="Determines durability requirements like min. cement content and max. water-binder ratio as per IS 456.", key="exposure")
        with c3: target_slump = st.slider("Target Slump (mm)", 25, 180, 100, 5, help="Specifies the desired consistency and workability of the fresh concrete.", key="target_slump")
        with c4: cement_choice = st.selectbox("Cement Type", CONSTANTS.CEMENT_TYPES, index=1, help="Select the type of cement used. Each option has distinct cost and CO₂ emission factors.", key="cement_choice")
        if enable_hpc:
            st.markdown("---"); st.subheader("🧪 High Performance Concrete (HPC) Options")
            hpc_col1, hpc_col2 = st.columns(2)
            with hpc_col1:
                max_sf_frac = st.slider("Max Silica Fume Fraction", 0.0, 0.15, 0.10, 0.01, help="Maximum silica fume as fraction of binder content")
                use_hpc_presets = st.checkbox("Use HPC Low w/b Presets", True, help="Allow w/b ratios as low as 0.25 for HPC grades")
            with hpc_col2: hpc_wb_min = st.slider("HPC Min w/b Ratio", 0.25, 0.40, 0.30, 0.01, disabled=not use_hpc_presets, help="Minimum water-binder ratio for HPC optimization")
            ...
            if not silica_fume_in_library: st.warning("⚠️ Silica fume not found in materials library. HPC mixes will use placeholder values for CO₂ and cost factors.", icon="🔬")
        st.markdown("---"); st.subheader("Aggregate Properties & Geometry")
        a1, a2, a3 = st.columns(3)
        with a1: nom_max = st.selectbox("Nominal Max. Aggregate Size (mm)", [10, 12.5, 20, 40], index=2, help="Largest practical aggregate size, influences water demand.", key="nom_max")
        with a2: agg_shape = st.selectbox("Coarse Aggregate Shape", list(CONSTANTS.AGG_SHAPE_WATER_ADJ.keys()), index=0, help="Shape affects water demand; angular requires more water than rounded.", key="agg_shape")
        with a3: fine_zone = st.selectbox("Fine Aggregate Zone (IS 383)", ["Zone I","Zone II","Zone III","Zone IV"], index=1, help="Grading zone as per IS 383. This is crucial for determining aggregate proportions per IS 10262.", key="fine_zone")
        st.markdown("---"); st.subheader("Admixtures & Quality Control")
        d1, d2 = st.columns(2)
        with d1: use_sp = st.checkbox("Use Superplasticizer (PCE)", True, help="Chemical admixture to increase workability or reduce water content.", key="use_sp")
        with d2: qc_level = st.selectbox("Quality Control Level", list(CONSTANTS.QC_STDDEV.keys()), index=0, help="Assumed site quality control, affecting the target strength calculation (f_target = fck + 1.65 * S).", key="qc_level")
        st.markdown("---"); st.subheader("Optimization Settings")
        o1, o2 = st.columns(2)
        with o1: purpose = st.selectbox("Design Purpose", list(purpose_profiles_data.keys()), index=0, key="purpose_select", help=purpose_profiles_data.get(st.session_state.get("purpose_select", "General"), {}).get("description", "Select the structural element."))
        with o2: optimize_for = st.selectbox("Single-Objective Priority", ["CO₂ Emissions", "Cost"], index=0, help="Choose whether to optimize the mix for cost or CO₂ footprint (used if Composite Optimization is disabled).", key="optimize_for_select")
        optimize_cost = (optimize_for == "Cost")
        enable_purpose_optimization = st.checkbox("Enable Purpose-Based Composite Optimization", value=(purpose != 'General'), key="enable_purpose", help="Optimize for a composite score balancing CO₂, Cost, and Purpose-Fit. If unchecked, uses the 'Single-Objective Priority' above.")
        ...
        st.markdown("---"); st.subheader("File Uploads (Sieve Analysis & Lab Data)"); st.caption("These files are for analysis and optional calibration, not core mix design input.")
        f1, f2, f3 = st.columns(3)
        with f1: fine_csv = st.file_uploader("Fine Aggregate Sieve CSV", type=["csv"], key="fine_csv", help="CSV with 'Sieve_mm' and 'PercentPassing' columns.")
        with f2: coarse_csv = st.file_uploader("Coarse Aggregate Sieve CSV", type=["csv"], key="coarse_csv", help="CSV with 'Sieve_mm' and 'PercentPassing' columns.")
        with f3: lab_csv = st.file_uploader("Lab Calibration Data CSV", type=["csv"], key="lab_csv", help="CSV with `grade`, `exposure`, `slump`, `nom_max`, `cement_choice`, and `actual_strength` (MPa) columns.")
        st.markdown("---")
    ...
    if enable_hpc:
        ...
        if use_hpc_presets: calib_wb_min = max(calib_wb_min, hpc_wb_min)
    purpose_weights = purpose_profiles_data['General']['weights']
    if enable_purpose_optimization and purpose != 'General':
    ...
    if run_button:
        ...
        if enable_calibration_overrides:
            calibration_kwargs = {
                "wb_min": calib_wb_min, "wb_steps": calib_wb_steps, "max_flyash_frac": calib_max_flyash_frac,
                "max_ggbs_frac": calib_max_ggbs_frac, "scm_step": calib_scm_step, "fine_fraction_override": calib_fine_fraction
            }
        ...
        inputs = { 
            "grade": grade, "exposure": exposure, "cement_choice": cement_choice, "nom_max": nom_max, "agg_shape": agg_shape,
            "target_slump": target_slump, "use_sp": use_sp, "optimize_cost": optimize_cost, "qc_level": qc_level,
            "fine_zone": fine_zone, "material_props": material_props, "purpose": purpose, "enable_purpose_optimization": enable_purpose_optimization,
            "purpose_weights": purpose_weights, "optimize_for": optimize_for, "enable_hpc": enable_hpc, "calibration_kwargs": calibration_kwargs
        }
    ...
    if st.session_state.get('run_generation_manual', False):
        ...
        run_generation_logic(inputs=st.session_state.final_inputs, emissions_df=emissions_df, costs_df=costs_df, purpose_profiles_data=purpose_profiles_data, st_progress=progress_bar)
        st.session_state.run_generation_manual = False
    if 'results' in st.session_state and st.session_state.results["success"]:
        ...
        st.markdown("---"); st.subheader("Mix Report Navigation")
        ...
        if selected_tab == "📊 **Overview**":
            ...
            if opt_meta.get("purpose", "General") != "General" or opt_meta.get("enable_hpc", False):
                ...
                if opt_meta.get("enable_hpc", False):
                    ...
                    if "purpose_metrics" in opt_meta and opt_meta["purpose_metrics"].get("pumpability_assessment"): c_h3.metric("📊 Pumpability", opt_meta["purpose_metrics"]["pumpability_assessment"])
                c_p1, c_p2, c_p3 = st.columns(3)
                ...
                if "composite_score" in opt_meta and not pd.isna(opt_meta["composite_score"]): c_p3.metric("🎯 Composite Score", f"{opt_meta.get('composite_score', 0.0):.3f}", help="Normalized score (lower is better).")
            st.markdown("---")
        ...
        elif selected_tab == "🏗️ **Baseline Mix**": display_mix_details("🏗️ Standard OPC Baseline Mix Design", base_df, base_meta, inputs['exposure'])
            ...
            if trace:
                ...
                if not feasible_mixes.empty:
                    ...
                    if not pareto_df.empty:
                        alpha = st.slider("Prioritize Sustainability (CO₂) ↔ Cost", min_value=0.0, max_value=1.0, value=current_alpha, step=0.05, help="Slide towards Sustainability to prioritize low CO₂, or towards Cost to prioritize low price. The green diamond will show the best compromise on the Pareto Front for your chosen preference.", key="pareto_slider_alpha")
                        ...
                        pareto_df_norm['score'] = (alpha * pareto_df_norm['norm_co2'] + (1 - alpha) * pareto_df_norm['norm_cost'] + pareto_df_norm['norm_penalty'] * 0.2)
                        best_compromise_mix = pareto_df_norm.loc[pareto_df_norm['score'].idxmin()]
                        fig, ax = plt.subplots(figsize=(10, 6))
                        ax.scatter(feasible_mixes["cost"], feasible_mixes["co2"], c=feasible_mixes["purpose_penalty"], cmap='viridis', alpha=0.5, label='All Feasible Mixes', zorder=1)
                        cbar = plt.colorbar(ax.collections[0], ax=ax); cbar.set_label('Purpose Penalty')
                        ...
                        ax.set_title("Pareto Front of Feasible Concrete Mixes (Color = Purpose Penalty)"); ax.grid(True, linestyle='--', alpha=0.6); ax.legend()
                        st.pyplot(fig)
            ...
            with st.expander("🔬 View Optimizer Trace (Advanced)"):
                if trace:
                    ...
                    st.dataframe(trace_df.style.apply(lambda s: [style_feasible_cell(v) for v in s], subset=['feasible']).format({
                        "feasible": lambda v: "✅" if v else "❌", "wb": "{:.3f}", "flyash_frac": "{:.2f}", "ggbs_frac": "{:.2f}", "sf_frac": "{:.2f}", "co2": "{:.1f}", "cost": "{:.1f}",
                        "purpose_penalty": "{:.2f}", "composite_score": "{:.4f}", "norm_co2": "{:.3f}", "norm_cost": "{:.3f}", "norm_purpose": "{:.3f}",
                    }), use_container_width=True)
    ...
    elif 'results' in st.session_state and not st.session_state.results["success"]: pass
...
def main():
    ...
    if "current_tab" not in st.session_state: st.session_state.current_tab = "📊 **Overview**"
    ...
    with st.sidebar:
        if st.session_state.chat_mode:
            card_title = "🤖 CivilGPT Chat Mode"; card_desc = "Converse with the AI to define mix requirements."; card_icon = "💬"; is_chat_mode = True
        else: card_title = "⚙️ Manual/Prompt Mode"; card_desc = "Use the detailed input sections to define your mix."; card_icon = "📝"; is_chat_mode = False
