...
E       if ok: msgs = [f"Fine aggregate conforms to IS 383 for {zone}."]
        return ok, msgs
...
            "Optimization Objective", ["CO₂ Emissions", "Cost"], index=0,
...
E           calib_scm_step = st.slider("SCM fraction step (scm_step)", 0.05, 0.25, 0.10, 0.05, help="Step size for testing different SCM replacement percentages.")
        
        st.sidebar.markdown("---")
...
          _E "purpose_weights": purpose_weights, "optimize_for": optimize_for,
...
E               st.session_state.run_generation_manual = False
            else:
                st.session_state.run_generation_manual = True
                st.session_state.final_inputs = inputs
        
...
IC                     pareto_df_norm['norm_cost'] = 0.0 if (cost_max - cost_min) == 0 else (pareto_df_norm['cost'] - cost_min) / (cost_max - cost_min)
...
  _E     elif selected_tab == "📋 **QA/QC & Gradation**":
...
E                     m2.metric(label="Root Mean Squared Error (RMSE)", value=f"{error_metrics['Root Mean Squared Error (MPa)']:.2f} MPa")
...
E                     ax.set_xlabel("Actual Lab Strength (MPa)"); ax.set_ylabel("Predicted Target Strength (MPa)")
...
E       st.session_state.chat_results_displayed = False
...
            st.session_state.chat_history = []
          s   st.session_state.chat_inputs = {}
...
ci   # --- 3. CHAT-TRIGGERED GENERATION (RUNS BEFORE UI) ---
...
II         "fine_zone": "Zone II", "material_props": default_material_props,
...
ci       inputs["enable_purpose_optimization"] = (inputs["purpose"] != 'General')
...
    if chat_mode:
s       run_chat_interface(purpose_profiles_data)
    else:
