...
class CONSTANTS:
    ...
    EXP_WB_LIMITS = {"Mild": 0.60, "Moderate": 0.55, "Severe": 0.50, "Very Severe": 0.45, "Marine": 0.40}
    EXP_MIN_CEMENT = {"Mild": 300, "Moderate": 300, "Severe": 320, "Very Severe": 340, "Marine": 360}
    EXP_MIN_GRADE = {"Mild": "M20", "Moderate": "M25", "Severe": "M30", "Very Severe": "M35", "Marine": "M40"}
    ...
    BINDER_RANGES = {"M10": (220, 320), "M15": (250, 350), "M20": (300, 400), "M25": (320, 420), "M30": (340, 450), "M35": (360, 480), "M40": (380, 500), "M45": (400, 520), "M50": (420, 540)}
    CA_FRAC_BY_ZONE = {10: {"Zone I": 0.50, "Zone II": 0.48, "Zone III": 0.46, "Zone IV": 0.44}, 12.5: {"Zone I": 0.59, "Zone II": 0.57, "Zone III": 0.55, "Zone IV": 0.53}, 20: {"Zone I": 0.66, "Zone II": 0.64, "Zone III": 0.62, "Zone IV": 0.60}, 40: {"Zone I": 0.71, "Zone II": 0.69, "Zone III": 0.67, "Zone IV": 0.65}}
    FA_ZONE_LIMITS = {"Zone I": {"10.0": (100,100),"4.75": (90,100),"2.36": (60,95),"1.18": (30,70),"0.600": (15,34),"0.300": (5,20),"0.150": (0,10)}, "Zone II": {"10.0": (100,100),"4.75": (90,100),"2.36": (75,100),"1.18": (55,90),"0.600": (35,59),"0.300": (8,30),"0.150": (0,10)}, "Zone III": {"10.0": (100,100),"4.75": (90,100),"2.36": (85,100),"1.18": (75,90),"0.600": (60,79),"0.300": (12,40),"0.150": (0,10)}, "Zone IV": {"10.0": (95,100),"4.75": (95,100),"2.36": (95,100),"1.18": (90,100),"0.600": (80,100),"0.300": (15,50),"0.150": (0,15)}}
    CA_LIMITS = {10: {"20.0": (100,100), "10.0": (85,100), "4.75": (0,20)}, 20: {"40.0": (95,100), "20.0": (95,100), "10.0": (25,55), "4.75": (0,10)}, 40: {"80.0": (95,100), "40.0": (95,100), "20.0": (30,70), "10.0": (0,15)}}
...
def load_default_excel(file_name):
    paths_to_try = [os.path.join(SCRIPT_DIR, file_name), os.path.join(SCRIPT_DIR, "data", file_name)]
...
def _normalize_material_value(s: str) -> str:
    ...
    synonyms = {"m sand": CONSTANTS.NORM_FINE_AGG, "msand": CONSTANTS.NORM_FINE_AGG, "m-sand": CONSTANTS.NORM_FINE_AGG, "fine aggregate": CONSTANTS.NORM_FINE_AGG, "sand": CONSTANTS.NORM_FINE_AGG, "20 coarse aggregate": CONSTANTS.NORM_COARSE_AGG, "20mm coarse aggregate": CONSTANTS.NORM_COARSE_AGG, "20 coarse": CONSTANTS.NORM_COARSE_AGG, "20": CONSTANTS.NORM_COARSE_AGG, "coarse aggregate": CONSTANTS.NORM_COARSE_AGG, "20mm": CONSTANTS.NORM_COARSE_AGG, "pce superplasticizer": CONSTANTS.NORM_SP, "pce superplasticiser": CONSTANTS.NORM_SP, "pce": CONSTANTS.NORM_SP, "opc 33": "opc 33", "opc 43": "opc 43", "opc 53": "opc 53", "ppc": "ppc", "fly ash": CONSTANTS.NORM_FLYASH, "ggbs": CONSTANTS.NORM_GGBS, "water": CONSTANTS.NORM_WATER}
...
def _normalize_columns(df, col_map):
    canonical_cols = list(dict.fromkeys(col_map.values()))
    ...
    rename_dict = {}
    for variant, canonical in col_map.items():
        if variant in norm_cols:
            original_col_name = norm_cols[variant]
            if canonical not in rename_dict.values(): rename_dict[original_col_name] = canonical
...
def _minmax_scale(series: pd.Series) -> pd.Series:
    ...
    if pd.isna(min_val) or pd.isna(max_val) or (max_val - min_val) == 0: return pd.Series(0.0, index=series.index, dtype=float)
    return (series - min_val) / (max_val - min_val)

@st.cache_data
def load_purpose_profiles(): return CONSTANTS.PURPOSE_PROFILES

def evaluate_purpose_specific_metrics(candidate_meta: dict) -> dict:
    try:
        ...
        return {"estimated_modulus_proxy (MPa)": round(modulus_proxy, 0), "shrinkage_risk_index": round(shrinkage_risk_index, 2), "pavement_fatigue_proxy": round(fatigue_proxy, 2)}
    except Exception: return {"estimated_modulus_proxy (MPa)": None, "shrinkage_risk_index": None, "pavement_fatigue_proxy": None}

@st.cache_data
def compute_purpose_penalty_vectorized(df: pd.DataFrame, pp: dict) -> pd.Series:
...
@st.cache_data
def load_data(mat_file=None, em_file=None, cost_file=None):
    def _safe_read(file, default_names, col_map):
        ...
        if file is not None:
            try:
                ...
                st.warning(f"Could not read uploaded file {getattr(file, 'name', 'N/A')}: {e}")
        ...
        if df is None or df.empty:
            ...
            if df is None: df = pd.DataFrame(columns=list(dict.fromkeys(col_map.values())))

        df = _normalize_columns(df, col_map)
        if "Material" in df.columns: df["Material"] = df["Material"].astype(str).str.strip()
        return df

    mat_df = _safe_read(mat_file, ["materials_library.csv", "data/materials_library.csv"], CONSTANTS.MATERIALS_COL_MAP)
    em_df = _safe_read(em_file, ["emission_factors.csv", "data/emission_factors.csv"], CONSTANTS.EMISSIONS_COL_MAP)
    cost_df = _safe_read(cost_file, ["cost_factors.csv", "data/cost_factors.csv"], CONSTANTS.COSTS_COL_MAP)

    if mat_df.empty or "Material" not in mat_df.columns: st.warning("Could not load 'materials_library.csv'. Using empty library.", icon="ℹ️")
    if em_df.empty or "Material" not in em_df.columns or "CO2_Factor(kg_CO2_per_kg)" not in em_df.columns: st.warning("⚠️ Could not load 'emission_factors.csv'. CO2 calculations will be zero.")
    if cost_df.empty or "Material" not in cost_df.columns or "Cost(₹/kg)" not in cost_df.columns: st.warning("⚠️ Could not load 'cost_factors.csv'. Cost calculations will be zero.")

    return mat_df, em_df, cost_df
...
def _merge_and_warn(main_df: pd.DataFrame, factor_df: pd.DataFrame, factor_col: str, warning_session_key: str, warning_prefix: str) -> pd.DataFrame:
    if factor_df is not None and not factor_df.empty and factor_col in factor_df.columns:
        ...
        if missing_items:
            if warning_session_key not in st.session_state: st.session_state[warning_session_key] = set()
...
def pareto_front(df, x_col="cost", y_col="co2"):
    ...
    for _, row in sorted_df.iterrows():
        if row[y_col] < last_y:
            ...
            last_y = row[y_col]
    return pd.DataFrame(pareto_points).reset_index(drop=True) if pareto_points else pd.DataFrame(columns=df.columns)
...
def reasonable_binder_range(grade: str): return CONSTANTS.BINDER_RANGES.get(grade, (300, 500))

@st.cache_data
def _get_ca_frac_base(nom_max_mm: float, fa_zone: str) -> float:
    return CONSTANTS.CA_FRAC_BY_ZONE.get(nom_max_mm, {}).get(fa_zone, 0.62)

@st.cache_data
def get_ca_frac(nom_max_mm: float, fa_zone: str, wb_ratio: float) -> float:
    base_fraction = _get_ca_frac_base(nom_max_mm, fa_zone)
    correction = ((0.50 - wb_ratio) / 0.05) * 0.01
    return max(0.4, min(0.8, base_fraction + correction))

@st.cache_data
def get_ca_frac_vectorized(nom_max_mm: float, fa_zone: str, wb_ratio_series: pd.Series) -> pd.Series:
    base_fraction = _get_ca_frac_base(nom_max_mm, fa_zone)
...
def run_lab_calibration(lab_df):
    ...
    for _, row in lab_df.iterrows():
        try:
            ...
            results.append({"Grade": grade, "Exposure": row.get('exposure', 'N/A'), "Slump (mm)": row.get('slump', 'N/A'), "Lab Strength (MPa)": actual_strength, "Predicted Target Strength (MPa)": predicted_strength, "Error (MPa)": predicted_strength - actual_strength})
    ...
    return results_df, {"Mean Absolute Error (MPa)": mae, "Root Mean Squared Error (MPa)": rmse, "Mean Bias (MPa)": bias}
...
def simple_parse(text: str) -> dict:
    ...
    if re.search("Marine", text, re.IGNORECASE): result["exposure"] = "Marine"
        ...
        for exp in CONSTANTS.EXP_WB_LIMITS.keys():
            if exp != "Marine" and re.search(exp, text, re.IGNORECASE): result["exposure"] = exp; break
    slump_match = re.search(r"(\d{2,3})\s*mm\s*(?:slump)?|slump\s*(?:of\s*)?(\d{2,3})\s*mm", text, re.IGNORECASE)
    if slump_match: result["target_slump"] = int(slump_match.group(1) or slump_match.group(2))
...
def parse_user_prompt_llm(prompt_text: str) -> dict:
    if not st.session_state.get("llm_enabled", False) or client is None: return simple_parse(prompt_text)
    system_prompt = f"""You are an expert civil engineer. Extract concrete mix design parameters from the user's prompt. Return ONLY a valid JSON object. Do not include any other text or explanations. If a value is not found, omit the key. Valid keys and values: - "grade": (String) Must be one of {list(CONSTANTS.GRADE_STRENGTH.keys())} - "exposure": (String) Must be one of {list(CONSTANTS.EXP_WB_LIMITS.keys())}. "Marine" takes precedence over "Severe". - "cement_type": (String) Must be one of {CONSTANTS.CEMENT_TYPES} - "target_slump": (Integer) Slump in mm (e.g., 100, 125). - "nom_max": (Float or Integer) Must be one of [10, 12.5, 20, 40] - "purpose": (String) Must be one of {list(CONSTANTS.PURPOSE_PROFILES.keys())} - "optimize_for": (String) Must be "CO2" or "Cost". - "use_superplasticizer": (Boolean)"""
    try:
        resp = client.chat.completions.create(model="mixtral-8x7b-32768", messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt_text}], temperature=0.0, response_format={"type": "json_object"})
        ...
        if parsed_json.get("exposure") in CONSTANTS.EXP_WB_LIMITS: cleaned_data["exposure"] = parsed_json["exposure"]
...
def evaluate_mix(components_dict, em_df, cost_df=None):
    ...
    df = _merge_and_warn(comp_df, em_df, "CO2_Factor(kg_CO2_per_kg)", "warned_emissions", "No emission factors found for")
    df["CO2_Emissions (kg/m3)"] = df["Quantity (kg/m3)"] * df["CO2_Factor(kg_CO2_per_kg)"]
    df = _merge_and_warn(df, cost_df, "Cost(₹/kg)", "warned_costs", "No cost factors found for")
...
def compute_aggregates(cementitious, water, sp, ca_frac, nom_max_mm, density_fa=2650.0, density_ca=2700.0):
    vol_cem, vol_wat, vol_sp = cementitious / 3150.0, water / 1000.0, sp / 1200.0
    ...
    vol_coarse, vol_fine = vol_agg * ca_frac, vol_agg * (1.0 - ca_frac)
    return float(vol_fine * density_fa), float(vol_coarse * density_ca)

def compute_aggregates_vectorized(binder_series, water_scalar, sp_series, ca_frac_series, nom_max_mm, density_fa, density_ca):
    vol_cem = binder_series / 3150.0
    vol_wat = water_scalar / 1000.0
    vol_sp = sp_series / 1200.0
    vol_air = CONSTANTS.ENTRAPPED_AIR_VOL.get(int(nom_max_mm), 0.01)
    vol_agg = (1.0 - (vol_cem + vol_wat + vol_sp + vol_air)).clip(lower=0.60)
    vol_coarse, vol_fine = vol_agg * ca_frac_series, vol_agg * (1.0 - ca_frac_series)
    return vol_fine * density_fa, vol_coarse * density_ca

def compliance_checks(mix_df, meta, exposure):
    checks = {}
    try: checks["W/B ≤ exposure limit"] = float(meta["w_b"]) <= CONSTANTS.EXP_WB_LIMITS[exposure]
    except: checks["W/B ≤ exposure limit"] = False
    try: checks["Min cementitious met"] = float(meta["cementitious"]) >= float(CONSTANTS.EXP_MIN_CEMENT[exposure])
    ...
    derived = {"w/b used": round(float(meta.get("w_b", 0.0)), 3), "cementitious (kg/m³)": round(float(meta.get("cementitious", 0.0)), 1), "SCM % of cementitious": round(100 * float(meta.get("scm_total_frac", 0.0)), 1), "total mass (kg/m³)": round(total_mass, 1) if "Quantity (kg/m3)" in mix_df.columns else None, "water target (kg/m³)": round(float(meta.get("water_target", 0.0)), 1), "cement (kg/m³)": round(float(meta.get("cement", 0.0)), 1), "fly ash (kg/m³)": round(float(meta.get("flyash", 0.0)), 1), "GGBS (kg/m³)": round(float(meta.get("ggbs", 0.0)), 1), "fine agg (kg/m³)": round(float(meta.get("fine", 0.0)), 1), "coarse agg (kg/m³)": round(float(meta.get("coarse", 0.0)), 1), "SP (kg/m³)": round(float(meta.get("sp", 0.0)), 2), "fck (MPa)": meta.get("fck"), "fck,target (MPa)": meta.get("fck_target"), "QC (S, MPa)": meta.get("stddev_S")}
    if "purpose" in meta and meta["purpose"] != "General": derived.update({"purpose": meta["purpose"], "purpose_penalty": meta.get("purpose_penalty"), "composite_score": meta.get("composite_score"), "purpose_metrics": meta.get("purpose_metrics")})
...
def get_compliance_reasons_vectorized(df: pd.DataFrame, exposure: str) -> pd.Series:
    limit_wb = CONSTANTS.EXP_WB_LIMITS[exposure]
    limit_cem = CONSTANTS.EXP_MIN_CEMENT[exposure]
...
def sieve_check_fa(df: pd.DataFrame, zone: str):
    try:
        limits, ok, msgs = CONSTANTS.FA_ZONE_LIMITS[zone], True, []
...
def sieve_check_ca(df: pd.DataFrame, nominal_mm: int):
    try:
        limits, ok, msgs = CONSTANTS.CA_LIMITS[int(nominal_mm)], True, []
        for sieve, (lo, hi) in limits.items():
            row = df.loc[df["Sieve_mm"].astype(str) == sieve]
            if row.empty: ok = False; msgs.append(f"Missing sieve size: {sieve} mm."); continue
            p = float(row["PercentPassing"].iloc[0])
...
@st.cache_data
def _get_material_factors(materials_list, em_df, cost_df):
    norm_map = {m: _normalize_material_value(m) for m in materials_list}
    norm_materials = list(set(norm_map.values()))
    co2_factors_dict = {}
    if em_df is not None and not em_df.empty and "CO2_Factor(kg_CO2_per_kg)" in em_df.columns:
        em_df_norm = em_df.copy().drop_duplicates(subset=["Material_norm"]).set_index("Material_norm")
        co2_factors_dict = em_df_norm["CO2_Factor(kg_CO2_per_kg)"].to_dict()
    cost_factors_dict = {}
    if cost_df is not None and not cost_df.empty and "Cost(₹/kg)" in cost_df.columns:
        cost_df_norm = cost_df.copy().drop_duplicates(subset=["Material_norm"]).set_index("Material_norm")
        cost_factors_dict = cost_df_norm["Cost(₹/kg)"].to_dict()
...
def generate_mix(grade, exposure, nom_max, target_slump, agg_shape, fine_zone, em_df, cost_df, cement_choice, mat_props, use_sp=True, sp_reduction=0.18, optimize_cost=False, wb_min=0.35, wb_steps=6, max_flyash_frac=0.3, max_ggbs_frac=0.5, scm_step=0.1, fine_fraction_override=None, purpose='General', purpose_profile=None, purpose_weights=None, enable_purpose_optimization=False, st_progress=None):
    ...
    density_fa, density_ca = mat_props['sg_fa'] * 1000, mat_props['sg_ca'] * 1000
    ...
    co2_factors, cost_factors = _get_material_factors(materials_to_calc, em_df, cost_df)
    ...
    ca_frac = 1.0 - fine_fraction_override if fine_fraction_override is not None and fine_fraction_override > 0.3 else get_ca_frac_vectorized(nom_max, fine_zone, grid_df['w_b'])
    grid_df['coarse_agg_fraction'] = ca_frac
    ...
    water_delta_fa_series, grid_df['fine_wet'] = aggregate_correction_vectorized(mat_props['moisture_fa'], grid_df['fine_ssd'])
    water_delta_ca_series, grid_df['coarse_wet'] = aggregate_correction_vectorized(mat_props['moisture_ca'], grid_df['coarse_ssd'])
    ...
    grid_df['co2_total'] = (grid_df['cement'] * co2_factors.get(norm_cement_choice, 0.0) + grid_df['flyash'] * co2_factors.get(CONSTANTS.NORM_FLYASH, 0.0) + grid_df['ggbs'] * co2_factors.get(CONSTANTS.NORM_GGBS, 0.0) + grid_df['water_final'] * co2_factors.get(CONSTANTS.NORM_WATER, 0.0) + grid_df['sp'] * co2_factors.get(CONSTANTS.NORM_SP, 0.0) + grid_df['fine_wet'] * co2_factors.get(CONSTANTS.NORM_FINE_AGG, 0.0) + grid_df['coarse_wet'] * co2_factors.get(CONSTANTS.NORM_COARSE_AGG, 0.0))
    grid_df['cost_total'] = (grid_df['cement'] * cost_factors.get(norm_cement_choice, 0.0) + grid_df['flyash'] * cost_factors.get(CONSTANTS.NORM_FLYASH, 0.0) + grid_df['ggbs'] * cost_factors.get(CONSTANTS.NORM_GGBS, 0.0) + grid_df['water_final'] * cost_factors.get(CONSTANTS.NORM_WATER, 0.0) + grid_df['sp'] * cost_factors.get(CONSTANTS.NORM_SP, 0.0) + grid_df['fine_wet'] * cost_factors.get(CONSTANTS.NORM_FINE_AGG, 0.0) + grid_df['coarse_wet'] * cost_factors.get(CONSTANTS.NORM_COARSE_AGG, 0.0))
    ...
    grid_df['feasible'] = (grid_df['w_b'] <= w_b_limit) & (grid_df['binder'] >= min_cem_exp) & (grid_df['scm_total_frac'] <= 0.50) & ((grid_df['total_mass'] >= 2200.0) & (grid_df['total_mass'] <= 2600.0))
    ...
    if feasible_candidates_df.empty: return None, None, grid_df.rename(columns={"w_b": "wb", "cost_total": "cost", "co2_total": "co2"}).to_dict('records')
    ...
    best_mix_dict = {cement_choice: best_meta_series['cement'], "Fly Ash": best_meta_series['flyash'], "GGBS": best_meta_series['ggbs'], "Water": best_meta_series['water_final'], "PCE Superplasticizer": best_meta_series['sp'], "Fine Aggregate": best_meta_series['fine_wet'], "Coarse Aggregate": best_meta_series['coarse_wet']}
    best_df = evaluate_mix(best_mix_dict, em_df, cost_df)
    
    best_meta = best_meta_series.to_dict()
    best_meta.update({"cementitious": best_meta_series['binder'], "water_target": target_water, "fine": best_meta_series['fine_wet'], "coarse": best_meta_series['coarse_wet'], "grade": grade, "exposure": exposure, "nom_max": nom_max, "slump": target_slump, "binder_range": (min_b_grade, max_b_grade), "material_props": mat_props, "purpose_metrics": evaluate_purpose_specific_metrics(best_meta)})
    ...
    if all(col in feasible_candidates_df.columns for col in score_cols): trace_df = trace_df.merge(feasible_candidates_df[score_cols], left_index=True, right_index=True, how='left')
    
    return best_df, best_meta, trace_df.to_dict('records')

def generate_baseline(grade, exposure, nom_max, target_slump, agg_shape, fine_zone, em_df, cost_df, cement_choice, mat_props, use_sp=True, sp_reduction=0.18, purpose='General', purpose_profile=None):
    w_b_limit = float(CONSTANTS.EXP_WB_LIMITS[exposure])
    min_cem_exp = float(CONSTANTS.EXP_MIN_CEMENT[exposure])
    ...
    ca_frac = get_ca_frac(nom_max, fine_zone, actual_wb)
    density_fa, density_ca = mat_props['sg_fa'] * 1000, mat_props['sg_ca'] * 1000
    
    fine_ssd, coarse_ssd = compute_aggregates(cementitious, water_target, sp, ca_frac, nom_max, density_fa, density_ca)
    water_delta_fa = (mat_props['moisture_fa'] / 100.0) * fine_ssd
    fine_wet = fine_ssd * (1 + mat_props['moisture_fa'] / 100.0)
    water_delta_ca = (mat_props['moisture_ca'] / 100.0) * coarse_ssd
    coarse_wet = coarse_ssd * (1 + mat_props['moisture_ca'] / 100.0)
    ...
    df = evaluate_mix(mix, em_df, cost_df)
    
    meta = {"w_b": actual_wb, "cementitious": cementitious, "cement": cementitious, "flyash": 0.0, "ggbs": 0.0, "water_target": water_target, "water_final": water_final, "sp": sp, "fine": fine_wet, "coarse": coarse_wet, "scm_total_frac": 0.0, "grade": grade, "exposure": exposure, "nom_max": nom_max, "slump": target_slump, "co2_total": float(df["CO2_Emissions (kg/m3)"].sum()), "cost_total": float(df["Cost (₹/m3)"].sum()), "coarse_agg_fraction": ca_frac, "material_props": mat_props, "binder_range": (min_b_grade, max_b_grade)}
    
    if purpose_profile is None: purpose_profile = CONSTANTS.PURPOSE_PROFILES.get(purpose, CONSTANTS.PURPOSE_PROFILES['General'])
            
    meta.update({"purpose": purpose, "purpose_metrics": evaluate_purpose_specific_metrics(meta), "purpose_penalty": compute_purpose_penalty_vectorized(pd.DataFrame([meta]), purpose_profile).iloc[0], "composite_score": np.nan})
    return df, meta

def apply_parser(user_text, current_inputs, use_llm_parser=False):
    if not user_text.strip(): return current_inputs, [], {}
    try: parsed = parse_user_prompt_llm(user_text) if use_llm_parser else simple_parse(user_text)
    except Exception as e: parsed = simple_parse(user_text)
    messages, updated = [], current_inputs.copy()
    if "grade" in parsed and parsed["grade"] in CONSTANTS.GRADE_STRENGTH: updated["grade"] = parsed["grade"]; messages.append(f"✅ Parser set Grade to **{parsed['grade']}**")
    if "exposure" in parsed and parsed["exposure"] in CONSTANTS.EXP_WB_LIMITS: updated["exposure"] = parsed["exposure"]; messages.append(f"✅ Parser set Exposure to **{parsed['exposure']}**")
    if "target_slump" in parsed: s = max(25, min(180, int(parsed["target_slump"]))); updated["target_slump"] = s; messages.append(f"✅ Parser set Target Slump to **{s} mm**")
    if "cement_choice" in parsed and parsed["cement_choice"] in CONSTANTS.CEMENT_TYPES: updated["cement_choice"] = parsed["cement_choice"]; messages.append(f"✅ Parser set Cement Type to **{parsed['cement_choice']}**")
    if "nom_max" in parsed and parsed["nom_max"] in [10, 12.5, 20, 40]: updated["nom_max"] = parsed["nom_max"]; messages.append(f"✅ Parser set Aggregate Size to **{parsed['nom_max']} mm**")
    if "purpose" in parsed and parsed["purpose"] in CONSTANTS.PURPOSE_PROFILES: updated["purpose"] = parsed["purpose"]; messages.append(f"✅ Parser set Purpose to **{parsed['purpose']}**")
...
def get_clarification_question(field_name: str) -> str:
    questions = {"grade": "What concrete grade do you need (e.g., M20, M25, M30)?", "exposure": f"What is the exposure condition? (e.g., {', '.join(CONSTANTS.EXP_WB_LIMITS.keys())})", "target_slump": "What is the target slump in mm (e.g., 75, 100, 125)?", "nom_max": "What is the nominal maximum aggregate size in mm (e.g., 10, 20, 40)?", "cement_choice": f"Which cement type would you like to use? (e.g., {', '.join(CONSTANTS.CEMENT_TYPES)})"}
...
def display_mix_details(title, df, meta, exposure):
    ...
    c_cols = st.columns(4)
    c_cols[0].metric("💧 Water/Binder Ratio", f"{meta['w_b']:.3f}")
    c_cols[1].metric("📦 Total Binder (kg/m³)", f"{meta['cementitious']:.1f}")
    c_cols[2].metric("🎯 Target Strength (MPa)", f"{meta['fck_target']:.1f}")
    c_cols[3].metric("⚖️ Unit Weight (kg/m³)", f"{df['Quantity (kg/m3)'].sum():.1f}")
    ...
    if purpose != "General":
        ...
        if "composite_score" in meta and not pd.isna(meta["composite_score"]): c_p3.metric("🎯 Composite Score", f"{meta.get('composite_score', 0.0):.3f}", help="Normalized score (lower is better).")
    ...
    is_feasible, fail_reasons, warnings, derived, _ = check_feasibility(df, meta, exposure)

    if is_feasible: st.success("✅ This mix design is compliant with IS code requirements.", icon="👍")
    else: st.error(f"❌ This mix fails {len(fail_reasons)} IS code compliance check(s): " + ", ".join(fail_reasons), icon="🚨")
...
def display_calculation_walkthrough(meta):
    ...
    - **Constraint:** Maximum w/b ratio for `{meta['exposure']}` exposure is `{CONSTANTS.EXP_WB_LIMITS[meta['exposure']]}`.
    ...
    - **Constraints Check:**
          - Min. for `{meta['exposure']}` exposure: `{CONSTANTS.EXP_MIN_CEMENT[meta['exposure']]}` kg/m³
...
def run_generation_logic(inputs: dict, em_df: pd.DataFrame, cost_df: pd.DataFrame, pp_data: dict, st_progress=None):
    try:
        min_grade_req = CONSTANTS.EXP_MIN_GRADE[inputs["exposure"]]
        ...
        calib_kwargs = inputs.get("calibration_kwargs", {})
        purpose = inputs.get('purpose', 'General')
        purpose_profile = pp_data.get(purpose, pp_data['General'])
        enable_purpose_opt = inputs.get('enable_purpose_optimization', False)
        purpose_weights = inputs.get('purpose_weights', pp_data['General']['weights'])
        if purpose == 'General': enable_purpose_opt = False
        
        if st_progress:
            st.info(f"🚀 Running {'composite' if enable_purpose_opt else 'single-objective'} optimization for **{purpose}**.", icon="🛠️" if enable_purpose_opt else "⚙️")
        
        fck = CONSTANTS.GRADE_STRENGTH[inputs["grade"]]
        S = CONSTANTS.QC_STDDEV[inputs.get("qc_level", "Good")]
        fck_target = fck + 1.65 * S
        
        opt_df, opt_meta, trace = generate_mix(inputs["grade"], inputs["exposure"], inputs["nom_max"], inputs["target_slump"], inputs["agg_shape"], inputs["fine_zone"], em_df, cost_df, inputs["cement_choice"], mat_props=inputs["material_props"], use_sp=inputs["use_sp"], optimize_cost=inputs["optimize_cost"], purpose=purpose, purpose_profile=purpose_profile, purpose_weights=purpose_weights, enable_purpose_optimization=enable_purpose_opt, st_progress=st_progress, **calib_kwargs)
        
        if st_progress: st_progress.progress(0.95, text="Generating baseline comparison...")
        base_df, base_meta = generate_baseline(inputs["grade"], inputs["exposure"], inputs["nom_max"], inputs["target_slump"], inputs["agg_shape"], inputs["fine_zone"], em_df, cost_df, inputs["cement_choice"], mat_props=inputs["material_props"], use_sp=inputs.get("use_sp", True), purpose=purpose, purpose_profile=purpose_profile)
        ...
        if opt_df is None or base_df is None:
            ...
            st.session_state.results = {"success": True, "opt_df": opt_df, "opt_meta": opt_meta, "base_df": base_df, "base_meta": base_meta, "trace": trace, "inputs": inputs, "fck_target": fck_target, "fck": fck, "S": S}
...
def run_chat_interface(pp_data: dict):
...
def run_manual_interface(pp_data: dict, mat_df: pd.DataFrame, em_df: pd.DataFrame, cost_df: pd.DataFrame):
    ...
    with st.expander("⚙️ Advanced Manual Input: Detailed Parameters and Libraries", expanded=False):
        ...
        with c1: grade = st.selectbox("Concrete Grade", list(CONSTANTS.GRADE_STRENGTH.keys()), index=4, help="Target characteristic compressive strength at 28 days.", key="grade")
        with c2: exposure = st.selectbox("Exposure Condition", list(CONSTANTS.EXP_WB_LIMITS.keys()), index=2, help="Durability requirements per IS 456.", key="exposure")
        with c3: target_slump = st.slider("Target Slump (mm)", 25, 180, 100, 5, help="Desired consistency.", key="target_slump")
        with c4: cement_choice = st.selectbox("Cement Type", CONSTANTS.CEMENT_TYPES, index=1, help="Cost and CO₂ emission factors.", key="cement_choice")
        
        st.markdown("---")
        st.subheader("Aggregate Properties & Geometry")
        a1, a2, a3 = st.columns(3)
        with a1: nom_max = st.selectbox("Nominal Max. Aggregate Size (mm)", [10, 12.5, 20, 40], index=2, help="Largest practical aggregate size.", key="nom_max")
        with a2: agg_shape = st.selectbox("Coarse Aggregate Shape", list(CONSTANTS.AGG_SHAPE_WATER_ADJ.keys()), index=0, help="Shape affects water demand.", key="agg_shape")
        with a3: fine_zone = st.selectbox("Fine Aggregate Zone (IS 383)", ["Zone I","Zone II","Zone III","Zone IV"], index=1, help="Grading zone per IS 383.", key="fine_zone")
        
        st.markdown("---")
        st.subheader("Admixtures & Quality Control")
        d1, d2 = st.columns(2)
        with d1: use_sp = st.checkbox("Use Superplasticizer (PCE)", True, help="Chemical admixture to increase workability.", key="use_sp")
        with d2: qc_level = st.selectbox("Quality Control Level", list(CONSTANTS.QC_STDDEV.keys()), index=0, help="Assumed site QC, affects target strength.", key="qc_level")

        st.markdown("---")
        st.subheader("Optimization Settings")
        o1, o2 = st.columns(2)
        with o1: purpose = st.selectbox("Design Purpose", list(pp_data.keys()), index=0, key="purpose_select", help=pp_data.get(st.session_state.get("purpose_select", "General"), {}).get("description", "Select the structural element."))
        with o2: optimize_for = st.selectbox("Single-Objective Priority", ["CO₂ Emissions", "Cost"], index=0, help="Used if Composite Optimization is disabled.", key="optimize_for_select")
        
        enable_purpose_optimization = st.checkbox("Enable Purpose-Based Composite Optimization", value=(purpose != 'General'), key="enable_purpose", help="Optimize for a composite score balancing CO₂, Cost, and Purpose-Fit.")
        optimize_cost = (optimize_for == "Cost")

        purpose_weights = pp_data['General']['weights']
        if enable_purpose_optimization and purpose != 'General':
            with st.expander("Adjust Composite Optimization Weights", expanded=True):
                default_weights = pp_data.get(purpose, {}).get('weights', pp_data['General']['weights'])
        ...
        if mat_df is not None and not mat_df.empty:
            try:
                mat_df_norm = mat_df.copy(); mat_df_norm['Material_norm'] = mat_df_norm['Material'].str.lower()
                fa_row = mat_df_norm[mat_df_norm['Material_norm'].str.contains('fine aggregate')].iloc[0]
                if 'SpecificGravity' in fa_row: sg_fa_default = float(fa_row['SpecificGravity'])
                if 'MoistureContent' in fa_row: moisture_fa_default = float(fa_row['MoistureContent'])
                ca_row = mat_df_norm[mat_df_norm['Material_norm'].str.contains('coarse aggregate')].iloc[0]
                if 'SpecificGravity' in ca_row: sg_ca_default = float(ca_row['SpecificGravity'])
                if 'MoistureContent' in ca_row: moisture_ca_default = float(ca_row['MoistureContent'])
        ...
        m1, m2 = st.columns(2)
        with m1: st.markdown("###### Fine Aggregate"); sg_fa = st.number_input("Specific Gravity (FA)", 2.0, 3.0, sg_fa_default, 0.01, key="sg_fa_manual"); moisture_fa = st.number_input("Free Moisture Content % (FA)", -2.0, 5.0, moisture_fa_default, 0.1, key="moisture_fa_manual")
        with m2: st.markdown("###### Coarse Aggregate"); sg_ca = st.number_input("Specific Gravity (CA)", 2.0, 3.0, sg_ca_default, 0.01, key="sg_ca_manual"); moisture_ca = st.number_input("Free Moisture Content % (CA)", -2.0, 5.0, moisture_ca_default, 0.1, key="moisture_ca_manual")
        ...
        with st.expander("Calibration & Tuning (Developer)", expanded=False):
            enable_calib_overrides = st.checkbox("Enable calibration overrides", False, key="enable_calibration_overrides")
            c1, c2 = st.columns(2)
            with c1: calib_wb_min = st.number_input("W/B search minimum (wb_min)", 0.30, 0.45, 0.35, 0.01, key="calib_wb_min")
            with c2: calib_wb_steps = st.slider("W/B search steps (wb_steps)", 3, 15, 6, 1, key="calib_wb_steps")
            with c1: calib_max_flyash_frac = st.slider("Max Fly Ash fraction", 0.0, 0.5, 0.30, 0.05, key="calib_max_flyash_frac")
            with c2: calib_max_ggbs_frac = st.slider("Max GGBS fraction", 0.0, 0.5, 0.50, 0.05, key="calib_max_ggbs_frac")
            with c1: calib_scm_step = st.slider("SCM fraction step (scm_step)", 0.05, 0.25, 0.10, 0.05, key="calib_scm_step")
            with c2: calib_fine_fraction = st.slider("Fine Aggregate Fraction (fine_fraction) Override", 0.30, 0.50, 0.40, 0.01, key="calib_fine_fraction")
    
    # Consolidate inputs from session state (simplifies logic below)
    inputs = {k: st.session_state.get(k) for k in ["grade", "exposure", "cement_choice", "nom_max", "agg_shape", "target_slump", "use_sp", "qc_level", "fine_zone"]}
    inputs["purpose"] = st.session_state.get("purpose_select", "General")
    inputs["optimize_for"] = st.session_state.get("optimize_for_select", "CO₂ Emissions")
    inputs["optimize_cost"] = (inputs["optimize_for"] == "Cost")
    inputs["enable_purpose_optimization"] = st.session_state.get("enable_purpose", False)
    inputs["material_props"] = {'sg_fa': st.session_state.get("sg_fa_manual", 2.65), 'moisture_fa': st.session_state.get("moisture_fa_manual", 1.0), 'sg_ca': st.session_state.get("sg_ca_manual", 2.70), 'moisture_ca': st.session_state.get("moisture_ca_manual", 0.5)}
    inputs["fine_fraction_override"] = st.session_state.get("calib_fine_fraction") if st.session_state.get("enable_calibration_overrides", False) else None
    if inputs["fine_fraction_override"] == 0.40 and not st.session_state.get("enable_calibration_overrides", False): inputs["fine_fraction_override"] = None
    
    calib_overrides = st.session_state.get("enable_calibration_overrides", False)
    inputs["calibration_kwargs"] = {"wb_min": st.session_state.get("calib_wb_min", 0.35) if calib_overrides else 0.35, "wb_steps": st.session_state.get("calib_wb_steps", 6) if calib_overrides else 6, "max_flyash_frac": st.session_state.get("calib_max_flyash_frac", 0.3) if calib_overrides else 0.3, "max_ggbs_frac": st.session_state.get("calib_max_ggbs_frac", 0.5) if calib_overrides else 0.5, "scm_step": st.session_state.get("calib_scm_step", 0.1) if calib_overrides else 0.1, "fine_fraction_override": inputs["fine_fraction_override"]}
    
    if inputs["enable_purpose_optimization"] and inputs["purpose"] != 'General':
        w_co2, w_cost, w_purpose = st.session_state.get("w_co2", 0.4), st.session_state.get("w_cost", 0.4), st.session_state.get("w_purpose", 0.2)
        total_w = w_co2 + w_cost + w_purpose
        inputs["purpose_weights"] = {"w_co2": w_co2 / total_w, "w_cost": w_cost / total_w, "w_purpose": w_purpose / total_w} if total_w > 0 else pp_data['General']['weights']
    else: inputs["purpose_weights"] = pp_data['General']['weights']
    ...
    if run_button:
        ...
        if st.session_state.user_text_input.strip():
            with st.spinner("🤖 Parsing your request..."):
                ...
                parsed_inputs, msgs, _ = apply_parser(st.session_state.user_text_input, inputs, use_llm_parser=use_llm_parser)
            if msgs: st.info(" ".join(msgs), icon="💡")
            
            required_fields = ["grade", "exposure", "target_slump", "nom_max", "cement_choice"]
            missing_fields = [f for f in required_fields if parsed_inputs.get(f) is None]

            if missing_fields:
                st.session_state.clarification_needed = True
                st.session_state.final_inputs = parsed_inputs
    ...
    if st.session_state.get('clarification_needed', False):
        ...
        with st.form("clarification_form"):
            ...
            CLARIFICATION_WIDGETS = {"grade": lambda v: st.selectbox("Concrete Grade", list(CONSTANTS.GRADE_STRENGTH.keys()), index=list(CONSTANTS.GRADE_STRENGTH.keys()).index(v) if v in CONSTANTS.GRADE_STRENGTH else 4), "exposure": lambda v: st.selectbox("Exposure Condition", list(CONSTANTS.EXP_WB_LIMITS.keys()), index=list(CONSTANTS.EXP_WB_LIMITS.keys()).index(v) if v in CONSTANTS.EXP_WB_LIMITS else 2), "target_slump": lambda v: st.slider("Target Slump (mm)", 25, 180, v if isinstance(v, int) else 100, 5), "cement_choice": lambda v: st.selectbox("Cement Type", CONSTANTS.CEMENT_TYPES, index=CONSTANTS.CEMENT_TYPES.index(v) if v in CONSTANTS.CEMENT_TYPES else 1), "nom_max": lambda v: st.selectbox("Nominal Max. Aggregate Size (mm)", [10, 12.5, 20, 40], index=[10, 12.5, 20, 40].index(v) if v in [10, 12.5, 20, 40] else 2)}

            cols = st.columns(min(len(missing_fields_list), 3))
            for i, field in enumerate(missing_fields_list):
                with cols[i % len(cols)]: current_inputs[field] = CLARIFICATION_WIDGETS[field](current_inputs.get(field))
    ...
    if st.session_state.get('run_generation_manual', False):
        ...
        run_generation_logic(st.session_state.final_inputs, em_df, cost_df, pp_data, st_progress=progress_bar)
        st.session_state.run_generation_manual = False
    ...
    if 'results' in st.session_state and st.session_state.results["success"]:
        ...
        if selected_tab == "📊 **Overview**":
            ...
            c3.metric("♻️ SCM Content", f"{opt_meta['scm_total_frac']*100:.0f}%", f"{base_meta['scm_total_frac']*100:.0f}% in baseline", help="Supplementary Cementitious Materials.")
            ...
            st.markdown("The blue line represents the **Pareto Front**—the set of most efficient mixes where you can't improve one objective without worsening the other.")
            if trace:
                ...
                if not feasible_mixes.empty:
                    ...
                    if not pareto_df.empty:
                        ...
                        full_compromise_mix = trace_df.loc[(trace_df['cost'] == best_compromise_mix['cost']) & (trace_df['co2'] == best_compromise_mix['co2'])].iloc[0]
                    ...
                    else: st.info("No Pareto front could be determined.", icon="ℹ️")
                else: st.warning("No feasible mixes were found by the optimizer.", icon="⚠️")
            ...
            with col1:
                ...
                else: st.info("Upload a Fine Aggregate CSV.", icon="ℹ️"); st.download_button("Download Sample Fine Agg. CSV", sample_fa_data, "sample_fine_aggregate.csv", "text/csv")
            with col2:
                ...
                else: st.info("Upload a Coarse Aggregate CSV.", icon="ℹ️"); st.download_button("Download Sample Coarse Agg. CSV", sample_ca_data, "sample_coarse_aggregate.csv", "text/csv")

            st.markdown("---"); with st.expander("📖 View Step-by-Step Calculation Walkthrough"): display_calculation_walkthrough(opt_meta)
            with st.expander("🔬 View Optimizer Trace (Advanced)"):
                if trace:
                    trace_df = pd.DataFrame(trace)
                    st.markdown("The table below shows every mix combination attempted by the optimizer.")
                    def style_feasible_cell(v): return 'background-color: #e8f5e9; color: #155724; text-align: center;' if v else 'background-color: #ffebee; color: #721c24; text-align: center;'
                    st.dataframe(trace_df.style.apply(lambda s: [style_feasible_cell(v) for v in s], subset=['feasible']).format({"feasible": lambda v: "✅" if v else "❌", "wb": "{:.3f}", "flyash_frac": "{:.2f}", "ggbs_frac": "{:.2f}", "co2": "{:.1f}", "cost": "{:.1f}", "purpose_penalty": "{:.2f}", "composite_score": "{:.4f}", "norm_co2": "{:.3f}", "norm_cost": "{:.3f}", "norm_purpose": "{:.3f}"}), use_container_width=True)
                    ...
                    ax.scatter(trace_df["cost"], trace_df["co2"], c=scatter_colors, alpha=0.6); ax.set_xlabel("Material Cost (₹/m³)"); ax.set_ylabel("Embodied Carbon (kg CO₂e/m³)"); ax.grid(True, linestyle='--', alpha=0.6); st.pyplot(fig)
            ...
            pdf_buffer = BytesIO()
            doc = SimpleDocTemplate(pdf_buffer, pagesize=(8.5*inch, 11*inch)); styles = getSampleStyleSheet()
            story = [Paragraph("CivilGPT Sustainable Mix Report", styles['h1']), Spacer(1, 0.2*inch)]
            summary_data = [["Metric", "Optimized Mix", "Baseline Mix"], ["CO₂ (kg/m³)", f"{opt_meta['co2_total']:.1f}", f"{base_meta['co2_total']:.1f}"], ["Cost (₹/m³)", f"₹{opt_meta['cost_total']:,.2f}", f"₹{base_meta['cost_total']:,.2f}"], ["w/b Ratio", f"{opt_meta['w_b']:.3f}", f"{base_meta['w_b']:.3f}"], ["Binder (kg/m³)", f"{opt_meta['cementitious']:.1f}", f"{base_meta['cementitious']:.1f}"], ["Purpose", f"{opt_meta.get('purpose', 'N/A')}", f"{base_meta.get('purpose', 'N/A')}"], ["Composite Score", f"{opt_meta.get('composite_score', 'N/A'):.3f}" if 'composite_score' in opt_meta and not pd.isna(opt_meta['composite_score']) else "N/A", "N/A"]]
            ...
            story.extend([Paragraph("Optimized Mix Proportions (kg/m³)", styles['h2']), opt_table]); doc.build(story); pdf_buffer.seek(0)
            ...
            with d1: st.download_button("✔️ Optimized Mix (CSV)", data=opt_df.to_csv(index=False).encode("utf-8"), file_name="optimized_mix.csv", mime="text/csv", use_container_width=True)
            ...
            if lab_csv_to_use is not None:
                try:
                    lab_csv_to_use.seek(0); lab_results_df = pd.read_csv(lab_csv_to_use); comparison_df, error_metrics = run_lab_calibration(lab_results_df)
                    if comparison_df is not None and not comparison_df.empty:
                        st.subheader("Error Metrics"); st.markdown("Comparing lab-tested 28-day strength against the IS code's required target strength.")
                        ...
                        fig, ax = plt.subplots(); ax.scatter(comparison_df["Lab Strength (MPa)"], comparison_df["Predicted Target Strength (MPa)"], alpha=0.7, label="Data Points")
                        lims = [np.min([ax.get_xlim(), ax.get_ylim()]), np.max([ax.get_xlim(), ax.get_ylim()])]
                        ax.plot(lims, lims, 'r--', alpha=0.75, zorder=0, label="Perfect Prediction (y=x)"); ax.set_xlabel("Actual Lab Strength (MPa)"); ax.set_ylabel("Predicted Target Strength (MPa)")
                        ax.set_title("Lab Strength vs. Predicted Target Strength"); ax.legend(); ax.grid(True); st.pyplot(fig)
                    else: st.warning("Could not process the uploaded lab data CSV. Check format and data.", icon="⚠️")
        ...
        st.markdown("""
        1.  **Input Requirements**: Describe your project needs.
        2.  **Select Purpose**: Choose your design purpose (e.g., 'Slab', 'Column').
        3.  **IS Code Compliance**: The app generates candidate mixes adhering to **IS 10262** and **IS 456**.
        4.  **Sustainability Optimization**: Calculates CO₂e, cost, and 'Purpose-Fit' for compliant mixes.
...
def main():
    ...
    st.markdown("""<style> .main .block-container { padding-top: 2rem; padding-bottom: 2rem; padding-left: 5rem; padding-right: 5rem; } .st-emotion-cache-1y4p8pa { max-width: 100%; } .stTextArea [data-baseweb=base-input] { border-color: #4A90E2; box-shadow: 0 0 5px #4A90E2; } [data-testid="chat-message-container"] { border-radius: 8px; padding: 0.75rem; margin-bottom: 0.5rem; } [data-testid="chat-message-container"] [data-testid="stMarkdown"] p { line-height: 1.6; } .mode-card { background-color: #1E1E1E; border-radius: 8px; padding: 15px; margin-bottom: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.5); border: 1px solid #333333; transition: all 0.3s; } .mode-card:hover { box-shadow: 0 6px 12px rgba(0, 0, 0, 0.7); border-color: #4A90E2; } .mode-card h4 { color: #FFFFFF; margin-top: 0; margin-bottom: 5px; } .mode-card p { color: #CCCCCC; font-size: 0.85em; margin-bottom: 10px; } [data-testid="stSidebarContent"] > div:first-child { padding-bottom: 0rem; } </style>""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown(f"""<div class="mode-card"><h4><span style='font-size: 1.2em; margin-right: 10px;'>{card_icon}</span>{card_title}</h4><p>{card_desc}</p></div>""", unsafe_allow_html=True)
        
        chat_mode_toggle_val = st.session_state.get("chat_mode") if llm_is_ready else False
        chat_mode = st.toggle(f"Switch to {'Manual' if is_chat_mode else 'Chat'} Mode", value=chat_mode_toggle_val, key="chat_mode_toggle_functional", help="Toggle to switch interfaces." if llm_is_ready else "Chat Mode requires a valid GROQ_API_KEY.", disabled=not llm_is_ready, label_visibility="collapsed")
    ...
    # --- 3. LOAD DATA (before interfaces) ---
    mat_df, em_df, cost_df = load_data(st.session_state.get("materials_csv"), st.session_state.get("emissions_csv"), st.session_state.get("cost_csv"))

    # --- 4. CHAT-TRIGGERED GENERATION (Runs before UI) ---
    if st.session_state.get('run_chat_generation', False):
        ...
        default_mat_props = {'sg_fa': 2.65, 'moisture_fa': 1.0, 'sg_ca': 2.70, 'moisture_ca': 0.5}
        inputs = {"grade": "M30", "exposure": "Severe", "cement_choice": "OPC 43", "nom_max": 20, "agg_shape": "Angular (baseline)", "target_slump": 125, "use_sp": True, "optimize_cost": False, "qc_level": "Good", "fine_zone": "Zone II", "material_props": default_mat_props, "purpose": "General", "enable_purpose_optimization": False, "purpose_weights": pp_data['General']['weights'], "optimize_for": "CO₂ Emissions", "calibration_kwargs": {}, **chat_inputs}
        inputs["optimize_cost"] = (inputs.get("optimize_for") == "Cost")
        inputs["enable_purpose_optimization"] = (inputs.get("purpose") != 'General')
        if inputs["enable_purpose_optimization"]: inputs["purpose_weights"] = pp_data.get(inputs["purpose"], {}).get('weights', pp_data['General']['weights'])

        st.session_state.final_inputs = inputs
        with st.spinner("⚙️ Running IS-code calculations and optimizing..."):
            run_generation_logic(inputs=inputs, em_df=em_df, cost_df=cost_df, pp_data=pp_data, st_progress=None)

    # --- 5. RENDER UI (Chat or Manual) ---
    if chat_mode: run_chat_interface(pp_data)
    else: run_manual_interface(pp_data, mat_df, em_df, cost_df)
